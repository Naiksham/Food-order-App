...
const Cart = () => {
  ...
  const totalPrice = cartItems.reduce((total,items)=>total+items.qty*items.price,0);

  const navigate = useNavigate();
  return (
    <>
      <div className={`fixed right-0 top-0 w-full lg:w-[20vw] bg-white p-5  h-full mb-3 ${activeCart ? "translate-x-0" : "translate-x-full"} transition-all duration-500 z-50`}>
